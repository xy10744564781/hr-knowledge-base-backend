...
class QueryRephraser:
    ...
    def _build_rephrase_prompt(self, query: str) -> str:
        """构建重述提示词"""
        return f"""你是一个专业的查询优化助手。请判断用户的查询是否与人事管理相关，并进行相应处理。

**判断规则：**
- 如果查询与人事管理相关（如考勤、请假、薪资、培训、入职、离职等），则重述为更适合检索的形式
- 如果查询与人事管理无关（如数学计算、技术问题、常识问题等），则直接返回原始查询，不要修改
